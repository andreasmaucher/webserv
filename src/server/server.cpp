...
/*   Updated: 2024/10/23 16:14:15 by cestevez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int Server::setup(const std::string& port) {
    ...
    std::vector<std::string> request_array(INIT_FD_SIZE);
            ...
            if (pfds[i].revents & POLLIN) { // Received a connection
                if (pfds[i].fd == listener_fd) { // If listener is ready to read, handle new connection
                    ...
                    if (nbytes <= 0) {
                        ...
                        buf[nbytes] = '\0';
                        printf("Received: %s\n", buf);
                        request_array[i] +=std::string(buf, nbytes);                           
                        // printf("EROOR HERE\n");

                        if (request_array[i].find("\r\n\r\n") != std::string::npos)
                        {
                            printf("OR EROOR HERE\n");

                            std::cout << "Full request from client " << i << " is: " << request_array[i] <<std::endl;
                            //PARSER COMES HERE
                        }
